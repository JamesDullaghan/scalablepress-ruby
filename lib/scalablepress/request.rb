...
module Scalablepress
  class Request
    ...
    # Username can be left blank, api key is used as password
    def set_basic_auth(method, api_key)
      method.basic_auth('', CGI.unescape(api_key))
    end
    ...
    def execute(target_base_url=nil, username: '', api_key: nil)
      base_uri = URI.parse(target_base_url)
      set_common_headers(net_http_method, base_uri)
      set_basic_auth(net_http_method, username, api_key)
      begin
        client(base.uri).start do |http|
          begin
            ...
            raise Scalablepress::ServiceUnavailableError.new('Service Unavailable [request timed out]')
          end
        end
      rescue Timeout::Error
        raise Scalablepress::ServiceConnectionError.new('Failed to connect to service [connection attempt timed out]')
      end
    end
